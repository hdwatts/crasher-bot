...
class Bot {
    ...
    static checkMessage(message) {
        ...
        if (messageText && /^\/sail.*/.test(messageText)) {
            const mention = message.attachments.find(att=>att.type=="mentions").user_ids[0];
            request({
                url: baseUrl + "/groups/" + groupid + "?token=" + token,
                method: "POST",
                ...
                const id = res.response.members.find(member=>member.user_id == mention).id
                request({
